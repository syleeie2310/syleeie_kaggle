...
set.seed(2310)
all_fe_train <- fread("F:/kaggle/Acquire Valued Shoppers Challenge/train/train_all_features.csv")
all_fe_test <- fread("F:/kaggle/Acquire Valued Shoppers Challenge/test/test_all_features.csv")
head(all_fe_train)
head(all_fe_test)
dim(all_fe_train)
dim(all_fe_test)
summary(all_fe_train)
...
mosaicplot(all_fe_train$days_from_lastdata_until_offerdate ~ all_fe_train$label, 
           main="days_from_lastdata_until_offerdate by label", shade=FALSE, 
           color=TRUE, xlab="days_from_lastdata_until_offerdate", ylab="label")
...
system.time(clf <- extraTrees(all_fe_train_tr1[,-1:-2], all_fe_train_tr1[,1], weights = sample_weights, ntree=100, numThreads=4))
...
param <- list(max.depth = 4, eta = 0.1, silent = 1, objective="binary:logistic")
